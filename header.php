...
<?php
// Page element called by every page to generate a consistent page expierence
...
function headerBar($title,$permission) {
    ...
    //Redirection if user_type check fails (page permission enforcement) admin bypasses check
    if($user_type != "admin"){
    ...
    // Header elements shown for customers and admin (admin has more in common with cust than employee)
	...
	if($user_type == "admin"){ //staff dropdown added for admin
    ...
    // Header elements shown for employees (with extra for managers)
